 ...
 * @LastEditTime: 2021-07-19 08:47:23
 * @LastEditors: liuct
-->
<template>
  <div class="video-con">
    <div class="video-main">
      <div
        v-for="item in videoModel"
        :key="item"
        :class="current === item ? 'video-active' : ''"
        :style="`width:${100 / Math.sqrt(videoModel)}%;height:${100 / Math.sqrt(videoModel)}%;`"
        @click="change(item)"
      ></div>
    </div>
...
export default {
  ...
  watch: {
    videoCode: {
      handler(val) {
        this.$nextTick(() => {
          this.getPreviewUrlById({ videoNo: val, type: this.videType });
        });
      },
      immediate: true
    },
  ...
  methods: {
    ...
    videoPlay(id, src) {
      const player = videojs(id, {}, function() {
        player.play();
      });
      if (this.videoPlays.length < this.videoModel) {
        this.videoPlays.push({ index: this.current, video: player, src: src });
      } else {
        this.videoPlays[this.videoModel - 1] = { index: this.current, video: player, src: src };
      }
    },
    addVideo(src) {
      const con = document.getElementsByClassName('video-active')[0];
      ...
      this.videoPlay(`myvideo${this.current}`, src);
      this.current = this.current < this.videoModel ? ++this.current : this.current;
    },
    async getPreviewUrlById(param) {
      const params = {
        ...param,
        protocol: 'hls'
      };
      const { resultData } = await getPreviewUrlById(params);
      if (resultData) {
        this.addVideo(`${resultData}`);
      } else {
    ...
    _setIntervalPlaly(steep) {
      ...
      if (typeof this.allVideoList[steep].length >= 0) {
        this.allVideoList[steep].forEach((item, i) => {
          this.getPreviewUrlById(item);
        });
      } else {
        this.getPreviewUrlById(this.allVideoList[steep]);
      }
    },
    stopAllPreview() {
      this.current = 1;
