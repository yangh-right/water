...
import VcTreeNode from './TreeNode.vue';
import VcVirtualNode from './TreeVirtualNode.vue';
...
export default {
  ...
  computed: {
    ...
    start() {
      if (this.visibleList.length && this.$route.path.indexOf('/report/historyAnalyse') > -1) {
        let idx = this.visibleList.findIndex(item => {
          return item.data.key === this.startCurrentKey;
        }) - 1;
        if(this.startCurrentKey === this.$route.query?.siteId){
          this.querySiteId = idx;
          this.$refs.vList.reset();
          // this.$nextTick(() => {
          //   this.$refs.vList.scrollToIndex(idx > -1 ? idx : 0);
          // });
          setTimeout(() => {
            this.$refs.vList.scrollToIndex(idx > -1 ? idx : 0);
          }, 20);
        }else{
          idx = this.querySiteId;
        }
        return idx > -1 ? idx : 0;
      }else {
        return 0;
  ...
  created() {
    ...
    this.$on('tree-node-drag-over', (event, treeNode) => {
      ...
      const iconPosition = dropNode.$el
        .querySelector('.vc-tree-node__expand-icon')
  ...
  methods: {
    flattenTree(datas) {
      return datas.reduce((conn, data) => {
        data.currentKey = this.currentNodeKey;
        conn.push(data);
