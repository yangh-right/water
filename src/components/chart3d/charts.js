...
const getPie3D = (
  pieData,
  { internalDiameterRatio, distance, alpha, pieHeight, opacity = 1 } = {}
) => {
  const series = [];
  let sumValue = 0;
  let startValue = 0;
  ...
  for (let i = 0; i < series.length; i++) {
    ...
    series[i].parametricEquation = getParametricEquation(
      ...
      false,
      k,
      pieHeight // series[i].pieData.value
    );
    startValue = endValue;
    ...
    tooltip: {
      formatter: params => {
        if (params.seriesName !== 'mouseoutSeries' && params.seriesName !== 'pie2d') {
          ...
          return `${params.marker} ${params.seriesName}: ${bfb}`;
        }
      }
    },
    // xAxis3D: {
...
const getHeight3D = (series, height) => {
  ...
  return 20; // (height * 25) / series[0].pieData.value;
};
