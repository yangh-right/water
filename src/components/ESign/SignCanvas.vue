<!--
 * @Author: chenzh
 * @Date: 2021-05-08 09:29:27
 * @LastEditTime: 2021-05-08 15:40:39
 * @LastEditors: chenzh
 * @Description:
 * @FilePath: /wisdomwatervue/src/components/ESign/SignCanvas.vue
-->
<template>
  <div>
    <canvas
  ...
  data() {
    ...
    return {
      ...
      config: {
        ...
        canvasWidth: 'auto', // canvas宽高 [Number] 可选
        canvasHeight: 240, // 高度  [Number] 可选
        isShowBorder: true, // 是否显示边框 [可选]
        bgColor: '#fff', // 背景色 [String] 可选 注:这背景色仅仅只是canvas背景,保存的图片仍然是透明的
        borderWidth: 1, // 网格线宽度  [Number] 可选
        borderColor: '#f1f1f1', // 网格颜色  [String] 可选
  ...
  methods: {
    init() {
      ...
      if (this.canvas) {
        ...
        let canvasWidth = this.$el.offsetWidth;
        ...
        this.config.canvasWidth =
          this.config.canvasWidth === 'auto' ? canvasWidth : this.config.canvasWidth;
    ...
    downloadSignImg(name) {
      ...
      this.saveFile(
        dataURL,
        name ? `${name}.${this.config.imgType}` : `${Date.parse(new Date())}.${this.config.imgType}`
      );
    },
    /**
     * 保存文件
