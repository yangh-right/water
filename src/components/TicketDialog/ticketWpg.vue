 ...
 * @LastEditTime: 2022-09-02 09:34:00
-->
<template>
  <w-modal
    id="ticket_modal"
...
import { createTicket } from '@/api/work';
...
export default {
  ...
  methods: {
    ...
    async queryTemplate() {
      ...
      this.templateId = (this.orderTemplateId && this.templateDisabled && this.typeDisabled) ? this.orderTemplateId : ((data.length ? data[0].code : ''));
    },
      ...
      if (alarmCreated) {
        ...
        let params = {
          ...
          createTicketDTO: {
            ...
            createTicketFormDTO: { ...formData, sysSource: config.systemCode }
          }
        };
        if (this.noType) {
          delete params.type
