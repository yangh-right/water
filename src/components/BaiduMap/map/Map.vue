...
export default {
    ...
    methods: {
        ...
        //             return new Promise(resolve => {
        //                 setTimeout(() => resolve(global.BMap), 1000)
        //             })
        //         }
        ...
        loadBaiDuMap() {
            ...
            return new Promise(function (resolve, reject) {
                ...
                window[callback] = function () {
                    ...
                    reject(new Error("BMap load error"));
                    }
                };

                if (window.__POWERED_BY_QIANKUN__) {
                    when(() => window.BMap && window.BMap.version).then(() => resolve(window.BMap));
                }
                }
            });
            },
        reset() {
            const { loadBaiDuMap, initMap } = this;
            loadBaiDuMap().then(initMap);
        },
    },
};
</script>
