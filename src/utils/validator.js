 ...
 */
 const regLong = /^[\-\+]?(0(\.\d{1,10})?|([1-9](\d)?)(\.\d{1,10})?|1[0-7]\d{1}(\.\d{1,10})?|180\.0{1,10})$/; // 经度正则
 const regLati = /^[\-\+]?((0|([1-8]\d?))(\.\d{1,10})?|90(\.0{1,10})?)$/; // 纬度正则
/**
 * 标题字符串：如角色标题
 * @param rule
 * @param value
...
// 经度必填
export const vRequiredLong = (rule, value, callback) => {
  if (!value) {
    ...
    let str = `${tempArr[0]}.${code}`;
    if (!regLong.test(str)) {
      callback(new Error('请输入格式正确的经度'));
    } else {
      callback();
    }
  }
...
// 纬度必填
export const vRequiredLati = (rule, value, callback) => {
  if (!value) {
 ...
 * @return {*}
 */
export function validPointMultiple(rule, value, callback) {
 ...
 * @param callback
 */
export const max50 = (rule, value, callback) => {
  const patrn = /^.{1,50}$/;
  if (value && !patrn.test(value)) {
    callback(new Error('已达到最大输入上限'));
  } else {
    callback();
  }
};
 ...
 * @param callback
 */
export const ticketReason = (rule, value, callback) => {
  const patrn = /^[\s\S]{1,254}$/;
  if (value && !patrn.test(value)) {
    callback(new Error('已达到最大输入上限'));
  } else {
    callback();
 ...
 */
export const bank = (rule, value, callback) => {
  const patrn = /^([1-9]{1})(\d{11}|\d{15}|\d{16}|\d{17}|\d{18})$/;
  if (value && !patrn.test(value)) {
 ...
 * @param callback
 */
export const haoreg = (rule, value, callback) => {
  const patrn = /(^(?:(?![IOZSV])[\dA-Z]){2}\d{6}(?:(?![IOZSV])[\dA-Z]){10}$)|(^\d{15}$)/;
  // 1位登记管理部门代码 + 1位机构类型代码 + 6位登记管理机关行政区划码 + 9位主体识别码（租住机构代码）+ 1位校验码
  if (value && !patrn.test(value)) {
 ...
 * @param callback
 */
export const max500 = (rule, value, callback) => {
  const patrn = /^.{1,500}$/;
  if (value && !patrn.test(value)) {
    callback(new Error('已达到最大输入上限'));
  } else {
 ...
 * @param callback
 */
export const max100 = (rule, value, callback) => {
  const patrn = /^.{1,100}$/;
  if (value && !patrn.test(value)) {
    callback(new Error('已达到最大输入上限'));
  } else {
 ...
 * @param callback
 */
export const max1000 = (rule, value, callback) => {
  const patrn = /^.{1,1000}$/;
  if (value && !patrn.test(value)) {
    callback(new Error('已达到最大输入上限'));
  } else {
 ...
 * @param callback
 */
export const files = (rule, value, callback) => {
  let flag = false;
  value.forEach(item => {
  ...
  if (flag) {
    callback(new Error('存在上传失败文件，请删除错误文件！'));
  } else {
    callback();
  }
};
