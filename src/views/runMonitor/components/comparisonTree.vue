  ...
  methods: {
    ...
    async getTree() {
      try {
        this.loading = true;
           this.treeData = [];
           let url = '/place/getLocationDeviceTree';
          Promise.all([getSysParamByCode('zzy_device'),this.$http.get(
          `${url}?waterPlantId=${this.waterPlantId}&keywords=${this.searchInput}`
        )])
        .then(result => {
          if(result[0].resultData){
            this.zzyDevices = result[0].resultData?.split(',');
            this.originalData = JSON.parse(JSON.stringify(result[1].resultData));

            this.treeData = this.originalData.filter(item => {
              item['children'] = item['children'].filter(child => {
                return this.zzyDevices.includes(child.id);
              })
