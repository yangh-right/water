<template>
  <div class="file-manage-view">
    <w-form-table-page>
      <template #form>
        ...
        >
          ...
          <template #footer>
            <w-button type="primary" @click="itemAuditing(false, true)" :disabled="!hasSelected">
              <i class="wpgicon icon-ic_add"></i>批量审批
            </w-button>
      ...
      <template #table>
        <w-info-table
          ...
          @downLoad="downLoad"
          @itemEdit="itemEdit"
          @itemAuditing="row => itemAuditing(row, false)"
    ...
    <add-model
      ...
      :modelType="modelType"
    ></add-model>
    <auditDialog ref="auditDialog" @handleAudit="handleAudit"></auditDialog>
  </div>
</template>
...
import { getPage, auditFile, deleteFile } from '@/api/fileManagement';
import { reportReviewcolumns } from './components/config';
...
export default {
  name: 'reportReview',
  components: { FactorySelect, Step, AddModel, auditDialog },
  data() {
    return {
  ...
  methods: {
    ...
    // 批量审批
    itemAuditing(row, isBatch = false) {
      this.isBatch = isBatch;
      if (row) {
        this.curRow = row;
      }
      this.$refs.auditDialog.visible = true;
    },
    async handleAudit(isAudit, remark) {
      let projectIds = this.isBatch ? this.selectedRowKeys : [this.curRow.id];
      let params = {
        idList: projectIds,
        auditStatus: isAudit ? 1 : 2,
        auditRemark: remark
      };
      let res = await auditFile(params);
      if (res.status === 'complete') {
        this.$message.success('审批成功');
        this.getList();
      }
      this.$refs.auditDialog.visible = false;
    },
    itemDelete(row) {
      this.$confirm({
        title: '确定要删除该条数据吗？',
