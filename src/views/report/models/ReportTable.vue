    ...
    <table-search>
      <w-row slot="form" :gutter="20" class="report-table-search">
        <w-form-model ref="form" :model="form">
          ...
          <w-col :span="5" v-if="item.isShowDevice && form.dataLevel == '0'">
          ...
          <w-col :span="5" v-if="item.isShowDevice && form.dataLevel == '-1'">
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        periodDateFormat: 'HH:00', // 分段格式化时间格式时间(表头):如HH:00
  ...
  methods: {
    ...
    async updateQuery() {
      ...
      this.form.dataLevel = this.item.dataLevel || this.item.dataLevel == '0' ? 0 : -1;
    ...
    async getTableData() {
      ...
      this.handleExcelParam();
      this.tableData = [];
      let sendData = { ...this.form, placeIds, deviceIds };
