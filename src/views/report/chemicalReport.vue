...
import {
  getWaterChemicalPage, // 水质报表分页查询
  exportWaterChemical // 导出
} from '@/api/report';
...
export default {
  ...
  data() {
    return {
      item: {
        ...
        queryData: getWaterChemicalPage,
        export: exportWaterChemical,
        isShowSum: true,
        isShowDevice: true,
        timeType: 'month',
        date: '',
  ...
  methods: {
    initQuery() {
      ...
      this.sysDictListByCode('chemicals');
    },
    async sysDictListByCode(code) {
      const res = await sysDictListByCode(code);
      this.item.plainOptions = res.resultData.map(item => {
        return {
          ...item,
          label: item.dictName,
