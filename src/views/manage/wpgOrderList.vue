    ...
    <table-search class="plan-manage-list">
      <w-row slot="form" :gutter="12">
        ...
        <w-col class="supply-float-right">
          <w-form-item>
            <w-button class="search-head-btn" type="primary" @click="handleAdd" v-if="recordPage">
...
export default {
  ...
  data() {
    return {
      ...
      ticketType: ['purificationInspect'], //工单类型
      rangeTime: [
        moment()
          .subtract(1, 'month')
          .format('YYYY-MM-DD'),
  ...
  computed: {
    ...
    selectTypeList() {
      ...
      this.orderTypeList.forEach(item => {
        if (this.ticketType.includes(item.code)) {
          arr.push(item);
        }
      });
      return arr;
    },
    //工单记录页面，表头工单状态、工单新增按钮隐藏
    recordPage() {
      return this.$route.query.ticketStatus ? false : true;
    }
  },
  watch: {},
  mounted() {
  ...
  methods: {
    ...
    initOptions() {
      if (this.$route.query) {
        ...
        if (this.$route.query.smallType) {
          this.ticketType = [this.$route.query.smallType];
        }
    ...
    handleAdd() {
      this.templateType = this.$route.query.smallType;
      this.templateData = {
        id: new Date().valueOf()
      };
    ...
    // 重置
    handleReset() {
      (this.options.keyword = ''), (this.options.priority = ''), (this.page.current = 1);
    ...
    getList() {
      ...
      let params = {
        ...this.options,
        createTimeStart: this.rangeTime[0]
          ? moment(this.rangeTime[0]).format('YYYY-MM-DD 00:00:00')
          : '',
        createTimeEnd: this.rangeTime[1]
          ? moment(this.rangeTime[1]).format('YYYY-MM-DD 23:59:59')
          : '',
        smallType: this.ticketType.join(','),
        page: {
