...
<template>
  <div class="alarm-center">
    <table-search>
      <w-form-model slot="form" layout="inline" ref="form" :model="form">
        <w-row :gutter="8" type="flex">
          ...
          <w-col v-bind="grid">
            <w-form-model-item label="报警类型" prop="alarmTypeCodes" :wrapperCol="{ flex: 1 }">
              <w-select v-model="form.alarmTypeCodes" mode="multiple" :max-tag-count="1" allowClear>
                <w-select-option
        ...
        <div class="table-wrapper">
          ...
          >
            ...
            <div slot="action" slot-scope="text, record" class="actions">
              ...
              <w-divider class="supply-float-left" type="vertical" />
              <a class="item2 supply-float-left" @click="handleImg(record)">查看图片</a>
            </div>
          </STable>
        </div>
        <div
    ...
    >
      <img v-if="previewImgUrl" style="width: 100%" :src="previewImgUrl" />
      <w-empty v-else size="small" class="empty" :description="`暂无图片`" />
    </w-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async initType() {
      ...
      this.form.alarmTypeCodes = this.alarmTypeList.map(data => data.value);
      this.refresh();
    },
    ...
    /* 报警图片*/
    async handleImg(record) {
      if (!record?.alarmHandlerDataId) {
        return;
      }
      ...
      try {
        ...
        if (status === 'complete') {
          ...
          this.previewImgUrl = resultData.fileIdUrlVOS?.[0]?.imageUrl ?? '';
    ...
    async loadData(parameter) {
      ...
      if (!this.params.alarmTypeCodes || this.params.alarmTypeCodes.length === 0) {
        return {
          total: 0,
          records: [],
...
.alarm-center {
  ...
  .alarm-con {
    ...
    > .table-wrapper {
      ...
      /deep/.supply-pro-table-title__toolbar {
        width: 100%;
        display: flex;
