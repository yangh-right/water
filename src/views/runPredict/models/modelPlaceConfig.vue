        ...
        >
          ...
          <template #action>
            <div class="supply-flex supply-justify-end">
              ...
              <w-button type="primary" icon="ic_search" @click="getPlaceModelList">
                <span>查询</span>
              </w-button>
            </div>
          </template>
...
import { fieldsList, placeColumns, searchForm } from './modelMeasureOption';
import configDetail from './modelPlaceModal.vue';
import portIn from '@/components/portinDialog/index.vue';
import { getPlaceModelPage, deletePlaceModel } from '@/api/runTime';
export default {
  name: 'PlaceConfig',
  ...
  methods: {
    ...
    async getPlaceModelList() {
      this.loading = true;
      const { resultData } = await getPlaceModelPage({ ...this.searchForm, page: this.page });
    ...
    handleAdd() {
      this.runTimeObj = 0;
      this.$nextTick(() => {
        if (this.$refs.placeModelEdit) {
          this.$refs.placeModelEdit.visible = true;
        }
      });
    },
    async handleDelete(row) {
      this.$confirm({
        ...
        content: '您确定要删除吗',
        onOk: async () => {
          let res = await deletePlaceModel(row.id);
