 ...
 * @LastEditTime: 2024-08-12 10:57:14
-->
<template>
  <div class="content">
  ...
  methods: {
    ...
    async getPointList() {
      ...
      let endTime = this.$moment()
        .add(24, 'hours')
        .format('YYYY-MM-DD HH:mm:ss');
      let method = this.tabVal === 1 ? getForecastAndHistory : getOutletWater;
      let { resultData, status } = await method(
        this.waterPlantId,
        this.modleType,
        startTime,
        endTime
    ...
    getPointData() {
      if (Object.keys(this.currentDevice).length > 0) {
        if (this.tabVal === 1) {
          ...
          let { dataAndTimeList, unit } = this.currentDevice;
          this.inWaterOption.yAxis[0].name = unit;
          this.inWaterOption.xAxis[0].data = dataAndTimeList.map(item => item.date?.slice(11, 16));
          this.inWaterOption.series[0].data = dataAndTimeList.map(item => item.pv);
          this.inWaterOption.series[1].data = dataAndTimeList.map(item => item.simulatePv);
        }
      }
    },
    handleTableData() {
      ...
      if (this.tabVal === 1) {
        ...
        this.tabs.forEach(item => {
          let { subChartVO, pointMemo, pointCode, unit } = item;
          let column = [
            {
              title: `实际值(${pointMemo}_${unit})`,
              width: 160,
              align: 'center',
              ellipsis: true,
        ...
        let length = this.tabs[0].dataAndTimeList.length;
        for (let k = 0; k < length; k++) {
          tableData.push({});
        }
        this.tabs.forEach(item => {
          let { dataAndTimeList, pointMemo, unit } = item;
          let column = [
            {
              title: `实际值(${pointMemo}_${unit})`,
              width: 160,
          ...
          moduleColumns.push(...column);
          dataAndTimeList.forEach((data, j) => {
            tableData[j]['id'] = uuid();
            tableData[j]['date'] = moment(data.date).format('HH:mm');
            tableData[j][`real_${pointMemo}`] = data.pv || '--';
            tableData[j][`pre_${pointMemo}`] = data.simulatePv || '--';
          });
        });
      }
