...
import { queryPointHistoryData, testWaterQuality } from '@/api/cockpit.js';
...
export default {
  ...
  methods: {
    ...
    handleOption() {
      const { unit, dataAndTimeList, cb = 0, gb = 0, pointName } = this.chartData;
      ...
      if (this.type === 'day') {
        ...
        // option.series[1].data = dataAndTimeList.map(item => item[`simulatePv`]);
      }
