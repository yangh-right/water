 ...
 * @LastEditors: wangyr
 * @LastEditTime: 2025-04-08 15:26:04
 * @Description:
-->
<template>
  <unit-card :cardTitle="cardTitle">
    ...
    <div :class="['list', themeConfig.className]" v-if="dataList.length">
      <div class="list__item" v-for="(item, index) in dataList" :key="item.dosageCode">
        ...
        <div class="value">
          <span class="val">{{ item.tonAmount }}</span>
          <span class="unit">{{ item.unit }}</span>
        </div>
        <div class="split"></div>
        <div class="rate">环比：{{ item.tonAmountRate }}%</div>
      </div>
    </div>
    <empty v-else></empty>
  </unit-card>
...
export default {
  ...
  methods: {
    async initData() {
      ...
      const { resultData, status } = await dosageOfReagent(params);
      if (status === 'complete') {
