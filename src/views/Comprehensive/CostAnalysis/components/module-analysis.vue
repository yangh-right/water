...
import { getEnergyTimeDataDeviceAnalysis } from '@/api/analysis.js';
...
export default {
  ...
  methods: {
    async getDeviceByCode() {
      ...
      if (status === 'complete') {
        this.deviceId = resultData?.industryDeviceId;
        if (this.deviceId) {
          this.getPointList();
        }
      }
    ...
    async getPointList() {
      ...
      let { resultData, status } = await getEnergyTimeDataDeviceAnalysis(params);
      this.loading = false;
      if (status === 'complete') {
    ...
    handleChartData() {
      if (Object.keys(this.currentDevice).length > 0) {
        let dataTime = [],
          relativeData = [];
        let { energyList, pointMemo, unit = 'kWh' } = this.currentDevice;
        if (energyList.length) {
          energyList.forEach(item => {
            dataTime.push(item.dateTime.slice(11, 16));
            relativeData.push(item.powerPv);
          });
        }
