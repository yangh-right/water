<template>
  <div class="device-detail-view full-view">
    ...
    <div class="detail-content" v-loading="loading">
            ...
            <w-row type="flex" justify="space-between">
              ...
              <w-col span="24">
                <w-form-model-item
                  ...
                  :wrapperCol="wrapperCol"
                >
                  <img-slider :isEdit="false" :imgList="addInfoForm.picUrl" />
                </w-form-model-item>
              </w-col>
            </w-row>
          </w-form-model>
            ...
            <w-row type="flex" justify="space-between">
              ...
              <w-col span="12">
                <w-form-model-item label="冗余情况：" label-for="selectType" prop="redundancy">
                  ...
                  <w-select v-else v-model="addInfoForm.redundancy" placeholder="请选择">
          ...
          >
            <w-row type="flex" justify="space-between">
              <w-col span="12">
                <w-form-model-item label="设备名称：" label-for="selectType" prop="equipmentName">
                  <div v-if="preview">
                    <span style="margin-right:6px">{{ addInfoForm.equipmentName }}</span>
                    <w-tooltip placement="right">
                      <template slot="title">
                        <img width="200" :src="imgUrl" alt="" />
                      </template>
                      <w-icon class="qrcode-brief" style type="qrcode" />
                    </w-tooltip>
                  </div>
                  <w-input
                    v-else
      ...
      <div v-else-if="contentType == 'extendInfo'" class="detail-content-left wbgc-card full-view">
        <div>
          <w-form-model
            ref="addInfoFormRule"
            :model="addInfoForm"
          ...
          >
            <w-row type="flex" justify="space-between">
              ...
              <w-col span="12">
                <w-form-model-item label="故障报修名称：" label-for="selectType" prop="faultRepairName">
              ...
              <w-col span="12">
                <w-form-model-item label="投运时间：" label-for="selectType" prop="commissioningDate">
              ...
              <w-col span="12">
                <w-form-model-item label="处理量：" label-for="selectType" prop="processingCapacity">
              ...
              <w-col span="12">
                <w-form-model-item label="设备参数：" label-for="selectType" prop="equipmentParameters">
              ...
              <w-col span="12">
                <w-form-model-item label="测定方式：" label-for="selectType" prop="measurementMethod">
              ...
              <w-col span="12">
                <w-form-model-item label="测量原理：" label-for="selectType" prop="measurementPrinciple">
      ...
      <div v-else-if="contentType == 'runAnalysis'" class="detail-content-left wbgc-card full-view">
        <w-form-model
          ref="addInfoFormRule"
          :model="addInfoForm"
          :rules="addInfoFormRule"
...
import DeviceLog from '@/views/Comprehensive/components/DeviceLog.vue';
...
export default {
  ...
  computed: {
    detailTitle() {
      return '设备档案 - ' + this.deviceDetail?.equipmentName || '';
    }
  },
  created() {
    this.getWaterPlant();
  ...
  methods: {
    ...
    updatePoint() {
      let keys = ['operatingTimeConfig', 'electricCurrentConfig']
      if (this.pointsList?.length) {
        keys.forEach(key => {
          let config = this.deviceDetail[key]
          ? String(this.deviceDetail[key]).split(',')
          : [];
          this.addInfoForm[key] = config
          .map(item => {
            let pointInfo = this.pointsList.find(point => point.value === item);
            let pointName = pointInfo ? pointInfo.label : '';
            return pointName;
          })
          .join('、');
        })
        let list = [];
        this.deviceDetail.devicePoints.forEach(device => {
