 ...
 * @LastEditTime: 2024-06-05 16:37:44
 * @Description:
-->
<template>
  <div class="supply-h-full">
    <div class="title">
      <div class="title__text">出水</div>
    </div>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      const { resultData, status } = (await this.toolConfig?.outWaterApi(params)) || {};
      if (status === 'complete') {
        this.pointList =
          resultData?.map(item => {
        ...
        this.option = this.pointList?.[0]?.option || genOutWaterOption();
