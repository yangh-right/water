 ...
 * @LastEditTime: 2023-07-04 19:42:17
-->
<template>
  <div class="content">
    ...
    <w-spin :spinning="loading" class="spin">
      <div class="chart-wrapper">
        <line-chart v-if="flag" :darkTheme="'light'" :option="outWaterOption" ref="inChart1"></line-chart>
...
import { outWaterOption } from './options';
export default {
  name: 'ChartData',
  props: {
    title: {
  ...
  methods: {
    ...
    async getPointList() {
      let params = {
        ...
        groupType: 'base_cockpit_outlet', //出水
      };
    ...
    async getPointData() {
      ...
      if (status === 'complete') {
        ...
        console.log('iii',forecastData)
        dataTime = Array.from(new Set(dataTime))
        this.outWaterOption.yAxis[0].name = data[0].unit;
          this.outWaterOption.xAxis[0].data = dataTime;
          this.outWaterOption.series[0].name = this.currentDevice.pointMemo;
          this.outWaterOption.series[0].data = historyData;
          this.outWaterOption.series[1].data = forecastData;
          this.flag = true;
      }
    },
  },
