 ...
 * @LastEditTime: 2025-04-16 11:09:54
 * @Description:
-->
<template>
  <div :class="['content', themeConfig.className]">
    <div class="echart-wrap">
      ...
      <div class="points-list">
        <div v-for="(item, i) in pointsArr" :key="item.key" class="point-tab">
          <div class="label">
            <div class="ico-react">
...
<script>
import { partyContent } from '@/api/cockpit';
    ...
    async getData() {
      ...
      if (this.partyTime === '1') {
        params.startDate = this.$moment()
          .startOf('month')
          .format('YYYY-MM-DD');
      } else if (this.partyTime === '2') {
        params.startDate = this.$moment()
          .subtract('months', 3)
          .format('YYYY-MM-DD');
      } else if (this.partyTime === '3') {
        params.startDate = this.$moment()
          .subtract('months', 6)
          .format('YYYY-MM-DD');
      } else {
      ...
      let { resultData, status } = await partyContent(params);
      if (status === 'complete') {
        this.pointsArr = resultData.map((item, index) => {
          return {
            ...item,
            itemStyle: {
