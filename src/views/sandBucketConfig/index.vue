...
import { getSandBucketList, deletePointStandard } from '@/api/reportRecord';
export default {
  name: 'ModelConfig',
  ...
  methods: {
    ...
    async getModelList() {
      ...
      const { resultData } = await getSandBucketList(params);
      this.dataSource = resultData.records;
      let sandBucket = this.dataSource.map(item => {
        return {
