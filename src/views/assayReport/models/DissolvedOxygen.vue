<template>
  <div class="base-info wfc1">
    <div class="form-title">溶解氧检测</div>
    <w-info-form
      ref="dissolvedOxygenForm"
      :model="form"
      :fields="baseFields"
      :layout="layout"
      :footer="null"
...
<script>
import { DISSOLVED_OXYGEN_FIELDS } from './config';
...
export default {
  ...
  data() {
    return {
      baseFields: DISSOLVED_OXYGEN_FIELDS,
      columns: [
        {
          title: '检测点',
  ...
  methods: {
    formValidate() {
      this.$refs['dissolvedOxygenForm'].validate(valid => {
        this.$emit('update:validate', !!valid);
      });
    },
    resetFields() {
      this.$refs['dissolvedOxygenForm'].$_resetFields();
    }
  }
};
</script>
