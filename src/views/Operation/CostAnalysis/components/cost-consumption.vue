  ...
  <div class="cost-consumption-view wbgc-card">
    ...
    <div class="content">
      <LineCard
        ...
        v-on="$listeners"
        :secondaryCostId="defaultSecondaryCost"
      />
      <div class="card" v-loading="loading">
        ...
        >
          ...
          <template #lastYear="{ text, row }">
            <span
              :style="{
                color: row.lastYearType === 1 ? 'red' : row.lastYearType === 2 ? 'green' : ''
              }"
              >{{ text | symbolData }}</span
            >
          </template>
          <template #lastMonth="{ text, row }">
            <span
              :style="{
                color: row.lastMonthType === 1 ? 'red' : row.lastMonthType === 2 ? 'green' : ''
              }"
              >{{ text | symbolData }}</span
            >
          </template>
    ...
    async getList() {
      ...
      try {
        let { resultData } = await this.$http.post(
          `/costAnalysis/getPlaceRatioList?costTypeId=${this.firstCostType.id}&smallTypeId=${this.defaultSecondaryCost}`,
