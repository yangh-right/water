    ...
    >
      <w-row :gutter="10">
        ...
        <template v-if="managementFlag">
          ...
          <w-col v-bind="grid">
            <w-form-model-item label="物品类别：">
              <w-tree-select
                v-model="informationFrom.articleType"
          ...
          <w-col v-bind="grid">
            <w-form-model-item label="查询单价：">
              <w-input-group compact>
                <w-input-number
                  v-model="informationFrom.startPrice"
                  :min="0"
                  style=" width: calc(50% - 17px); text-align: center"
                  :placeholder="'开始单价'"
                  :allowClear="true"
                />
                <w-input
                  style=" width: 33px; border-left: 0; pointer-events: none; "
                  class="range-ico"
                  placeholder="~"
                  disabled
                />
                <w-input-number
                  :class="['suffix-code']"
                  v-model="informationFrom.endPrice"
                  :min="0"
                  style="width: calc(50% - 17px); text-align: center; border-left: 0"
                  :placeholder="'结束单价'"
                  :allowClear="true"
                />
              </w-input-group>
        ...
        <w-col class="supply-float-right">
          <w-button class="mr_12 supply-top-1" @click="handleImport"
            ><w-icon type="ic_publish" />导入</w-button
          >
          <w-button class="mr_12 supply-top-1" @click="handleExport(0)"
            ><w-icon type="ic_file_download" />导出</w-button
          >
          <w-button class="mr_12 supply-top-1" @click="handleExport(1)"
          ...
          <w-button type="primary" class="mr_12 supply-top-1" @click="addInformation" icon="plus">
            新建</w-button
          >
          <w-button class="mr_12 supply-top-1" @click="managementReset" icon="reload"
            >重置</w-button
          >
          <w-button
            type="primary"
            ...
            >查询</w-button
          >
          <a class="mr_12 supply-top-1" @click="managementFlag = !managementFlag">
            <template v-if="!managementFlag">
              展开
              <w-icon type="down" />
            </template>
      ...
      >
        <w-row type="flex" justify="space-between">
          ...
          <w-col span="12">
            ...
            >
              <w-tree-select
                v-model="addInfoForm.materielTypeId"
                style="width: 200px"
                :dropdown-style="{ maxHeight: '200px', overflow: 'auto' }"
                :tree-data="articleTypeList"
                :replaceFields="replaceFields"
...
export default {
  ...
  methods: {
    ...
    handleExport(type) {
      ...
      vo.materielTypeId = this.informationFrom.articleType;
      vo.startPrice = this.informationFrom.startPrice;
      vo.endPrice = this.informationFrom.endPrice;
      method(vo).then(res => {
    ...
    editMaintain(row) {
      ...
      this.addInfoForm.materielTypeId = row.materielTypeId;
    ...
    getBaseMaterielList(parameter) {
      ...
      vo.materielName = this.informationFrom.articleName;
      vo.materielTypeId = this.informationFrom.articleType;
      vo.waterPlantId = this.informationFrom.waterPlantId;
      vo.startPrice = this.informationFrom.startPrice;
      vo.endPrice = this.informationFrom.endPrice;
      // }
      this.tableLoading = true;
    ...
    managementReset() {
      ...
      this.informationFrom.default = '';
      this.informationFrom.articleType = '';
      this.informationFrom.startPrice = '';
      this.informationFrom.endPrice = '';
      ...
      this.$refs.addInfoFormRule.validate(valid => {
        if (valid) {
          let vo = {
            ...
            materielTypeId: Number(this.addInfoForm.materielTypeId),
...
[data-wpg-theme='dark'] .range-ico {
  background: var(--supply-color-bg-tabelHead) !important;
}
.file-item {
  width: 120px;
  cursor: pointer;
