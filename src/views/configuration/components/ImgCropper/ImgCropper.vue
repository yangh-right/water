<template>
  <div v-show="myValue" class="vue-image-crop-upload">
    <div class="vicp-wrap">
      ...
      <div v-show="step == 1" class="vicp-step1">
        ...
        <div class="supply-text-third" style="margin: 10px 0;">{{ tip }}</div>
      ...
      <div v-if="step == 2" class="vicp-step2">
        <div class="vicp-crop">
          <div class="vicp-crop-left">
            ...
            <div v-if="isTailoring" class="vicp-range">
              <input
                type="range"
                :value="scale.range"
                step="1"
...
export default {
  props: {
    ...
    tipMassage: {
      type: String,
      default: '只能上传jpg/png文件，最佳大小112*44，且不超过2M'
    },
  ...
  data() {
    ...
    let allowImgFormat = ['jpg', 'jpeg', 'png', 'gif'];
    ...
    return {
      ...
      sourceImgContainer: {
        // sic
        width: 260,
        height: 210 // 如果生成图比例与此一致会出现bug，先改成特殊的格式吧，哈哈哈
      },
      // 原图展示属性
      scale: {
  ...
  created() {
    ...
    document.addEventListener('keyup', e => {
      if (this.myValue && (e.key == 'Escape' || e.keyCode == 27)) {
        this.off();
      }
    });
  },
  methods: {
    ...
    checkFile(file) {
      let that = this;
      let { lang, maxSize, imgFormat, withIcon } = that;
      // 仅限图片
      ...
      if (!withIcon && file.type.indexOf('x-icon') !== -1) {
        that.hasError = true;
        that.errorMsg = '不能上传ico图标';
        return false;
      }
      // 仅限icon
      if (imgFormat === 'x-icon') {
        if (file.type.indexOf('x-icon') === -1) {
          that.hasError = true;
          that.errorMsg = '仅限ico图片格式';
          return false;
        }
      }
      // 超出大小
    ...
    startCrop() {
      let that = this;
      let { width, height, ratio, scale, sourceImgUrl, sourceImgMasking, lang } = that;
      let sim = sourceImgMasking;
      let img = new Image();
      ...
      img.onload = function() {
        ...
        let y = 0;
        // 图片像素不达标
        if ((nWidth < width || nHeight < height) && this.WHVerification) {
          that.hasError = true;
    ...
    zoomImg(newRange) {
      let that = this;
      let { sourceImgMasking, sourceImgMouseDown, scale } = this;
      let { maxWidth, maxHeight, minWidth, minHeight, width, height, x, y, range } = scale;
      let sim = sourceImgMasking;
      // 蒙版宽高
    ...
    createImg(e) {
      ...
      if (this.isTailoring) that.createImgUrl = canvas.toDataURL(mime);
      if (!this.isTailoring) that.createImgUrl = that.sourceImgUrl;
    },
    // 预备上传
    prepareUpload() {
     ...
     */
    getImageTye() {
      let imgArr = this.createImgUrl.split(';');
      let imgArr2 = imgArr[0].split('/');
      if (imgArr2[1] === 'x-icon') imgArr2[1] = 'ico';
      return imgArr2[1];
    },
    // 上传图片
    upload() {
      ...
      let that = this;
      let { lang, mime, imgFormat, url, params, field, ki, createImgUrl, withCredentials } = this;
      let fmData = new FormData();

      imgFormat = this.getImageTye(); // 获取图片类型
      fmData.append(field, data2blob(createImgUrl, mime[imgFormat]), field + '.' + imgFormat);
      ...
      new Promise(function(resolve, reject) {
        ...
        client.addEventListener('progress', uploadProgress, false); // 监听进度
        // // 设置header
