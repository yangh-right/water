      ...
      <w-row :gutter="10">
        ...
        <w-col class="supply-float-right">
          <w-form-model-item label="" :labelCol="{ span: 0 }" :wrapperCol="{ span: 24 }">
            ...
            >
            <w-button type="primary" class="mr_12" @click="addInformation" icon="plus"
...
import { numValidator, handleNum, numValidator1, handlePrice } from './components/data.js';
import {
  deleteBaseMateriel,
  getBaseMaterielType,
  getDepotShelfList,
...
import Loop from './components/levelLoop';
...
export default {
  ...
  methods: {
      ...
      getBaseMaterielType().then(respionse => {
        ...
        function loopChangeTitle(obj) {
          obj.forEach(item => {
            ...
            item.selected = true;
            item.checked = false;
            if (item.data && item.data.materielTypeNo === 'huaxueyaoji') {
    ...
    managementReset() {
      ...
      this.informationFrom.materielTypeNo = 'huaxueyaoji';
