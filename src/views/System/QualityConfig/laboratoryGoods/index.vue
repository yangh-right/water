      ...
      <w-row :gutter="10">
        ...
        <w-col class="supply-float-right">
          <w-form-model-item label="" :labelCol="{ span: 0 }" :wrapperCol="{ span: 24 }">
            ...
            <w-button type="primary" class="mr_12" @click="addInformation" icon="plus"></w-button>
              新建</w-button
            >
    ...
    >
      ...
      >
        <w-row type="flex" justify="space-between">
          ...
          <w-col span="12">
            <w-form-model-item label="税率：" label-for="selectType" prop="taxRate">
              <w-input
                v-model="addInfoForm.taxRate"
                placeholder
                style="width: 200px"
                type="number"
                :min="1"
...
import {
  ...
  numValidator1,
  handlePrice
} from '../agentiaList/components/data.js';
import {
  deleteBaseMateriel,
  getBaseMaterielType,
  getDepotShelfList,
...
import SupplierEdit from '../agentiaList/components/SupplierEdit.vue';
import { sysDictListByCode } from '@/api/manage';
import Loop from '../agentiaList/components/levelLoop';
...
export default {
  ...
  data() {
    return {
      ...
      informationFrom: {
        ...
        articleId: '', // 物品编号
        articleName: '', // 物品名称
        materielTypeNo: 'shiyanshiwuzi', // 物品管理员
        default: '' // 默认仓库
      }, // 物料资料-查询数据
      informationColumns: [
        {
  ...
  methods: {
    ...
    handleExport(type) {
      ...
      vo.materielTypeNo = this.informationFrom.materielTypeNo;
      method(vo).then(res => {
      ...
      getBaseMaterielType().then(respionse => {
        ...
        function loopChangeTitle(obj) {
          obj.forEach(item => {
            ...
            item.selected = true;
            item.checked = false;
            if (item.data && item.data.materielTypeNo === 'shiyanshiwuzi') {
              vm.materielTypeId = item.data.id;
              vm.addInfoForm.materielTypeId = item.data.id;
            }
            if (item.children) {
              // item.disabled = true
    ...
    // 物品资料 - 增加
    addInformation() {
      this.addInfoForm = { taxRate: 1, materielTypeId: this.materielTypeId };
      this.addInfoModelLoading = true;
    ...
    managementReset() {
      ...
      this.informationFrom.materielTypeNo = 'shiyanshiwuzi';
...
@import '../agentiaList/warehouse-top.less';
.supplier-edit {
  cursor: pointer;
  position: absolute;
  right: -30px;
